(function () {
  ...
  var inferFrom = function inferFrom(doclet, node) {
    if (node.type === "VariableDeclaration") {
      ...
      doclet.name.setName(varDecl.id.name);
      inferKindFrom(doclet, varDecl.init);
      ...
      if (expr.type === "AssignmentExpression") {
        ...
        if (qname) doclet.name.assign(qname);
        inferKindFrom(doclet, expr.right);
      }
    } else if (node.type === "Property") {
      if (node.key.type === "Identifier") {
        /* { name : ... }
         * Declaration in an object literal. */
        doclet.name.setName(node.key.name);
        inferKindFrom(doclet, node.value);
      }
    }
  };
