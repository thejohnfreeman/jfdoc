  ...
  Parser.prototype.addSymbol = function addSymbol(symbol) {
    switch (symbol.kind) {
      case "file":
        ...
        symbol.decls = [];
        this.symbols.files.unshift(symbol);
        break;
      ...
      case "function":
        ...
        symbol.qualifiers.forEach(function (qualifier) {
          scope = this.pushScope(scope, qualifier);
        }, this);
        this.pushScope(scope, symbol.name, symbol);

        var fsym = this.symbols.files[0];
        if (fsym && fsym.filename === this.filename) {
          fsym.decls.push(symbol);
        }
        break;
