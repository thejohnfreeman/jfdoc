(function () {
  ...
  Parser.prototype.addSymbol = function addSymbol(symbol) {
    switch (symbol.kind) {
      case "file":
        this.symbols.files[0].tags = symbol;
        break;
      ...
      case "function":
        ...
        symbol.scopeQuals.forEach(function (qual) {
          scope = this.enterScope(scope, qual);
        }, this);
        this.pushScope(scope, symbol);

        this.symbols.files[0].decls.push(symbol);
        break;
  ...
  Parser.prototype.pushScope = function pushScope(outer, symbol) {
    outer.decls[symbol.baseName] = symbol;
  };

  /* The actions below are meant to be shared among tag parsers. Checking should
   * be moved here when possible. */
  Parser.prototype.setQualName = function setQualName(symbol, qualName) {
    if (qualName.length > 1) symbol.scopeQuals = qualName.slice(0, -1);
    symbol.baseName = qualName[qualName.length - 1];
  };
