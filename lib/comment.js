...
(function () {
  ...
  TreeIterator.prototype.childrenOf = function childrenOf(node) {
    switch (node.type) {
      ...
      case "ExpressionStatement": return this.childrenOf(node.expression);
      case "CallExpression": return this.childrenOf(node.callee);
      case "FunctionExpression": return this.childrenOf(node.body);
      case "BlockStatement": return node.body;
      case "AssignmentExpression": return [this.right];
  ...
  var attachComments = function attachComments(root) {
    ...
    root.comments.forEach(function (comment) {
      ...
      if (node) {
        assert.notStrictEqual(node, nodePrev,
          "forgot to implement childrenOf for " +
          (nodePrev ? nodePrev.type : "anything"));
        logAction("assigning", node, comment);
        node.comment = comment;
        comment.subject = node;
      } else {
        console.warn("loose comment after " +
          (nodePrev ? nodePrev.type : "whatever was last") + ": " +
          comment.value.slice(0, 10));
      }
