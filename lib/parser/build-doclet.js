define(function (require) {
  ...
  var buildDoclet = function buildDoclet(comment, node) {
    ...
    if (!comment && !this.config.infer) return;
    ...
    if (!comment && (!qname.getName() || kind === "unknown")) return;

    /* TODO: Handle nested names. For now, suppress diagnostics. */
    if (!comment && qname.isNested()) return;
    ...
    var doclet = new Doclet(description, /*isExplicit=*/true);
